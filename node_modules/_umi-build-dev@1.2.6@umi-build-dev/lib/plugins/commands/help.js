...
var _stringPrototype = _interopRequireDefault(require("string.prototype.padend"));
...
var _default = api => {
  ...
  function logMainHelp(helpInfo) {
    ...
    for (const name in commands) {
      ...
      if (opts.hide !== true) {
        console.log(`    ${_chalk.default.green((0, _stringPrototype.default)(name, padLength))}${opts.description || ''}`);
      }
    }
    ...
    if (!command) {
      ...
      if (opts.options) {
        ...
        for (const name in opts.options) {
          console.log(`    ${_chalk.default.green((0, _stringPrototype.default)(name, padLength))}${opts.options[name]}`);
        }
      }
