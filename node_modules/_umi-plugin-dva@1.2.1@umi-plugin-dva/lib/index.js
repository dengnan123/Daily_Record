...
function isPagesPath(path, api) {
  const paths = api.paths,
        winPath = api.winPath;
  return (0, _umiUtils.endWithSlash)(winPath(path)) === (0, _umiUtils.endWithSlash)(winPath(paths.absPagesPath));
}
...
app: window.g_app,
models: () => [
  ${models.map(model => `import(${opts.dynamicImport.webpackChunkName ? `/* webpackChunkName: '${(0, _umiUtils.chunkName)(paths.cwd, model)}' */` : ''}'${model}')`).join(',\r\n  ')}
],
      `.trim());
      }
